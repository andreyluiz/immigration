...
function PackageCard({ icon, title, description, price, payment }: PackageCardProps) {
  return (
    <div className="group relative bg-gradient-to-br from-background to-background/80 backdrop-blur-sm rounded-2xl p-8 shadow-lg border border-border/50 transition-all duration-300 hover:shadow-2xl hover:scale-105 hover:border-primary/30">
      ...
      <div className="relative z-10">
        ...
        <div className="text-foreground/70 mb-6 leading-relaxed whitespace-pre-line">{description}</div>
        <div className="pt-4 border-t border-border/30">
          <div className="text-3xl font-extrabold text-primary mb-1">{price}</div>
          <div className="text-sm text-foreground/60 font-medium">{payment}</div>
        </div>
      </div>
    </div>
  );
...
function ExtrasCard({ icon, title, description, price, payment }: PackageCardProps) {
  return (
    <div className="group relative bg-gradient-to-br from-secondary/10 to-secondary/5 backdrop-blur-sm rounded-2xl p-8 shadow-lg border border-secondary/30 transition-all duration-300 hover:shadow-2xl hover:scale-105">
      ...
      <div className="relative z-10">
        ...
        <h3 className="text-xl font-bold mb-3 text-foreground group-hover:text-secondary transition-colors">{title}</h3>
        <div className="text-foreground/70 mb-6 leading-relaxed whitespace-pre-line">{description}</div>
    ...
    <div className="relative py-20 md:py-32 overflow-hidden">
      ...
      <div className="relative container mx-auto px-4">
        ...
        <div className="max-w-4xl mx-auto mb-20 text-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-6 text-foreground">{t("overview.title")}</h2>
          <p className="text-xl text-foreground/80 leading-relaxed">{t("overview.description")}</p>
        </div>
        ...
        <div className="mb-20">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold mb-4">{t("premium_addon.title")}</h2>
            <p className="text-lg text-foreground/70 max-w-2xl mx-auto">{t("premium_addon.description")}</p>
          </div>
        ...
        <div className="relative bg-gradient-to-br from-muted/80 to-muted/40 backdrop-blur-sm rounded-3xl p-10 md:p-16 mb-20 border border-border/30">
          ...
          <div className="relative z-10">
            <h2 className="text-3xl md:text-4xl font-bold mb-12 text-center">{t("why_choose.title")}</h2>
            <div className="grid md:grid-cols-3 gap-8">
              {reasons.map((reason: string, index: number) => (
                <div key={`reason-${index + 1}`} className="group flex items-start p-6 rounded-2xl bg-background/50 backdrop-blur-sm border border-border/30 hover:bg-background/80 transition-all duration-300">
                  <div className="bg-gradient-to-br from-accent/20 to-accent/10 p-2 rounded-xl mr-4 group-hover:scale-110 transition-transform">
                    <Icon icon="mdi:check-circle" className="w-5 h-5 text-accent" />
                  </div>
                  <p className="text-foreground/80 leading-relaxed font-medium">{reason}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="text-center max-w-4xl mx-auto">
          <div className="bg-gradient-to-br from-background to-background/80 backdrop-blur-sm rounded-3xl p-10 md:p-16 border border-border/30 shadow-2xl">
            <h2 className="text-3xl md:text-4xl font-bold mb-6">{t("next_steps.title")}</h2>
            <p className="text-xl text-foreground/80 mb-8 leading-relaxed">{t("next_steps.description")}</p>
            <div className="mb-8">
              <a 
              ...
              >
                <Icon icon="mdi:email" className="w-5 h-5 group-hover:scale-110 transition-transform" />
                {t("next_steps.email")}
              </a>
            </div>
            <div className="inline-block">
              <Button asChild size="lg" className="text-lg flex items-center font-semibold shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105">
                <a href="/#contact-form" className="flex items-center justify-center w-full h-full">
